...
export default <RouterOptions>{
  routes: (_routes) => {
    ...
    if (subdomain.value) {
      const userRoute = _routes.filter(i => i.path.includes('/provider/:url'))
      const userRouteMapped = userRoute.map(i => ({
        ...i,
        path:
          i.path === '/provider/:url'
            ? i.path.replace('/provider/:url', '/')
            : i.path.replace('/provider/:url/', '/')
      }))
      console.log('User route ', userRoute) // es-lint-disable-line no-console
      console.log('Route mapped ', userRouteMapped) // es-lint-disable-line no-console
